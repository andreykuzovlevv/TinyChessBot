...
typedef enum { SNAKE_DIR_RIGHT, SNAKE_DIR_UP, SNAKE_DIR_LEFT, SNAKE_DIR_DOWN } SnakeDirection;
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    ...
    *appstate = as;
    ...
    if (!SDL_CreateWindowAndRenderer("examples/demo/snake", SDL_WINDOW_WIDTH, SDL_WINDOW_HEIGHT,
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_JOYSTICK_ADDED:
            if (joystick == NULL) {
                ...
                if (!joystick) {
                    SDL_Log("Failed to open joystick ID %u: %s", (unsigned int)event->jdevice.which,
                            SDL_GetError());
                }
            }
            break;
